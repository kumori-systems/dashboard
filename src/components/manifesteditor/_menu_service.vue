<template>
  <div class="sidebar-nav navbar-collapse">
    <ul class="nav in" id="side-menu">
      <li :ref="option.name" v-for="(option, index) in menuOptions" v-bind:key="index" :class="{active: option.name == active}" @click="updateActive(option.name, $event)" >
        <a href="#" @click="option.clear? cleanCurrent(option.target): null" :data-toggle="option.target ? 'modal':''" :data-target="option.target ? option.target : ''" class="menu-title activator">
          <i  v-bind:class="option.icon+' activator'"></i>
          {{ $t(option.name) }}
